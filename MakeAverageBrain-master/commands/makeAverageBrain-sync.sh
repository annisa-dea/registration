...
GJROOT="/home/emily/registration"
MUNGERDIR="/home/emily/cmtk/build/bin"
export REGBINDIR="/home/emily/cmtk/build/bin"
...
while [ $i -le $NOITERATIONS ]; do
	...
	#qsub -wd "$REGROOT/jobs" -sync yes -t 1-${NOIMAGES} -S /bin/bash -m eas -M ${LMBUSER}@lmb.internal -pe smp 4 
...
#qsub -wd "$REGROOT/jobs" -S /bin/bash -m eas -M ${LMBUSER}@lmb.internal -pe smp 8 
"$REGROOT/commands/symmetricOutput.sh" ${NEWREFPATH} ${SYMREFPATH} ${GJROOT} ${REGROOT}
